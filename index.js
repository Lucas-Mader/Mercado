...
function submit_market_info(nome_mercado, logo_mercado) {
...
function enviar_concluir_adicionar() {
    ...
    alert('Algo deu errado na adição do produto!');
}

function search_product(nome_produto) {
        ...
        for (let i = 0; i < produtos.length; i++) {
            if (produtos[i] != null) {
                if(produtos[i].nome_produto === nome_produto.value) {
                    document.getElementById('form_editar').innerHTML = 
                    '<p><label>Nome do produto: </label>' + produtos[i].nome_produto +
                    ...
                    '<p><input type="button" id="enviar_remover" value="Remover produto" onclick="remove_product()">' +
                    ' \\-||-/' +
                    '<input type="button" id="enviar_editar" value="Editar produto" onclick="edit_modal()"></p>';
                    found_item = true;
                    break;
                    }
                }
...
function remove_product() {
    ...
    if (produtos) {
        for (let i = 0; i < produtos.length; i++) {
            if (produtos[i] && produtos[i].nome_produto === nome_produto.value) {
                produtos.splice(i, 1);
...
function edit_modal() {
    ...
    if (produtos) {
        for (let i = 0; i < produtos.length; i++) {
            if (produtos[i] && produtos[i].nome_produto === nome_produto.value) {
                document.getElementById('form_editar').innerHTML = 
                '<p><h1><label id="editarProduto">Editar produto</label></h1></p>' +
...
function enviar_concluir_editar(nome_produto, categoria_produto, foto_produto, unitario_produto, localizacao_produto, foto_localizacao_produto, qtd_produto, qtd_produto_pack, preco_unitario_produto) {
    let list_json = localStorage.getItem('Produtos');
    if(list_json) {
        list_produtos = JSON.parse(list_json);
        let produto = {
    ...
    alert('Algo deu errado na edição do produto!');
}
...
function open_listar_tudo() {
    let div_lista = document.getElementById('listar');
    div_lista.innerHTML = '<div id="div_to_list">' + 
    '<input type="button" value="Parar de listar tudo" id="button_to_not_list" onclick="close_listar_tudo()">' + 
    '</div>';
    let produtos = JSON.parse(localStorage.getItem('Produtos'));
...
function close_listar_tudo() {
    let div_lista = document.getElementById('listar');
    div_lista.innerHTML = '<div id="div_to_list">' +
    '<input type="button" value="Listar tudo" id="button_to_list" onclick="open_listar_tudo()">' + 
    '</div>';
}
